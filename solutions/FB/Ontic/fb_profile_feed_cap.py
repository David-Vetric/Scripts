...
def main():
    ...
    print(f"\n🔍 Collecting Facebook feed posts for profileId={PROFILE_ID}\n")

    while True:
        data = make_request(end_cursor)

        results = data.get("results", {}) or {}
        feed = results.get("feed", []) or []
        count = len(feed)
        total_posts += count

        print(f"\n📄 Page {page} — {count} posts")

        for idx, post in enumerate(feed, start=1):
            message = post.get("message")
            preview = (message or "").replace("\n", " / ")
            print(f"   🔹 Post {idx}: {preview[:80]}")
