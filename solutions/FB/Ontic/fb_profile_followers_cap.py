...
PROFILE_ID = "4"
URL = f"{base_url}/facebook/v1/profiles/{PROFILE_ID}/followers"
...
def main():
    ...
    print(f"\n🔍 Collecting Facebook followers for profileId={PROFILE_ID}\n")
    ...
    while True:
        ...
        print(f"\n📄 Page {page} — {count} followers")

        for idx, follower in enumerate(results, start=1):
            name = follower.get("name")
            print(f"   🔹 Follower {idx}: {name}")
