...
PROFILE_ID = "4"
URL = f"{base_url}/facebook/v1/profiles/{PROFILE_ID}/tagged-media"
...
def main():
    page = 1
    total_tagged_posts = 0
    end_cursor = None

    print(f"\n🔍 Collecting Facebook tagged posts for profileId={PROFILE_ID}\n")

    while True:
        data = make_request(end_cursor)

        tagged_media = data.get("tagged_media", []) or []
        count = len(tagged_media)
        total_tagged_posts += count

        print(f"\n📄 Page {page} — {count} tagged posts")

        for idx, media in enumerate(tagged_media, start=1):
            media_id = media.get("media_id")
            print(f"   🔹 Tagged Post {idx}: media_id={media_id}")
    ...
    print(f"Total tagged posts collected: {total_tagged_posts}")
    print("\n🏁 Finished.\n")
